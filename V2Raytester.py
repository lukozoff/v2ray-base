import os, json, time, base64, asyncio, httpx, shutil, re, sys, zipfile, argparse
...
# --- НАСТРОЙКИ ---
VERSION = "1.4.1 Ultimate beta ver 1"
...
async def check_link(semaphore, geo_client, link, idx, pbar, results, wl, no_rename):
    global live_count
...
async def main():
    ...
    parser.add_argument("-n", "--no-rename", action="store_true", help="Keep original names and skip GeoIP")
    ...
    async with httpx.AsyncClient() as geo_client:
        with tqdm(total=len(unique_links), unit="cfg", desc="Ultimate Check") as pbar:
            tasks = [asyncio.create_task(check_link(semaphore, geo_client, link, i, pbar, results, wl, args.no_rename)) for i, link in enumerate(unique_links)]
